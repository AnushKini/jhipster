...
export class JobSeekerComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.jobSeekerService
            ...
            .subscribe(
                (res: HttpResponse<IJobSeeker[]>) => this.paginateJobSeekers(res.body, res.headers),
    ...
    trackId(index: number, item: IJobSeeker) {
        return item.id;
    }
    ...
    registerChangeInJobSeekers() {
        this.eventSubscriber = this.eventManager.subscribe('jobSeekerListModification', response => this.reset());
    }
    ...
    protected paginateJobSeekers(data: IJobSeeker[], headers: HttpHeaders) {
